...
class Statistics:
    ...
    staleRate = 0
    trans = 0
    infor_send_latency = 0
    trans_package_chain_latency = 0
    latency = 0
    total_size = 0
    block_used_rate = 0
    blockData = []
    ...
    def blocks_results():
        ...
        for b in c.global_chain:
            ...
            for i in b.transactions:
                temp.append(i)
            Statistics.trans += len(b.transactions)
            for j in range(len(b.transactions)):
                Statistics.total_size += b.transactions[j].size
        ...
        Statistics.infor_send_latency = Statistics.infor_send_latency / Statistics.trans
        Statistics.trans_package_chain_latency = Statistics.trans_package_chain_latency / Statistics.trans
        Statistics.latency = Statistics.infor_send_latency + Statistics.trans_package_chain_latency
        Statistics.block_used_rate = Statistics.total_size / (Statistics.mainBlocks * p.Bsize)
        Statistics.blockData = [Statistics.totalBlocks, Statistics.mainBlocks, Statistics.uncleBlocks,
                                Statistics.uncleRate, Statistics.staleBlocks, Statistics.staleRate, Statistics.trans,
                                Statistics.latency, Statistics.block_used_rate]
    ...
    def profit_results():
        ...
        for m in p.NODES:
            ...
            if p.model == 1:
                Statistics.profits[i][1] = m.stackPower
            else:
    ...
    def print_to_excel(fname):
        ...
        df1 = pd.DataFrame(
            {'Block Time': [p.Binterval], 'Block Propagation Delay': [p.Bdelay], 'No. Miners': [len(p.NODES) - 1],
        ...
        df2.columns = ['Total Blocks', 'Main Blocks', 'Uncle blocks', 'Uncle Rate', 'Stale Blocks', 'Stale Rate',
                       '# transactions', 'AverageLantency', '利用率']
